...
app.post('/api/clientes', async (c) => {
    ...
    run(`INSERT INTO clientes (telefono, nombre, direccion, indicaciones_entrega, tramo_delivery_id)
     VALUES (?, ?, ?, ?, ?)
     ON CONFLICT(telefono) DO UPDATE SET
       nombre = excluded.nombre,
       direccion = excluded.direccion,
       indicaciones_entrega = excluded.indicaciones_entrega,
       tramo_delivery_id = excluded.tramo_delivery_id`, [telefono, nombre, direccion, indicaciones_entrega, tramo_delivery_id]);
    ...
    try {
        ...
        if (telefono) {
         ...
         ON CONFLICT(telefono) DO UPDATE SET
           ...
           tramo_delivery_id = COALESCE(excluded.tramo_delivery_id, clientes.tramo_delivery_id)`, [telefono, nombre, direccion, indicaciones, tramo_delivery_id]);
        }
        run(`INSERT INTO pedidos (cliente_telefono, tipo_pedido, costo_delivery, total_cobrado, metodo_pago, descuento_porcentaje, direccion_entrega, fecha, estado)
       VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now', 'localtime'), 'pendiente')`, [telefono, tipo_pedido, costo_delivery, total_cobrado, metodo_pago, descuento_porcentaje, direccion]);
...
app.get('/api/pedidos/pendientes', (c) => {
  ...
  `);
    ...
    const resultados = pedidos.map((p) => ({ ...p, items: items.filter((i) => i.pedido_id === p.id) }));
    return c.json(resultados);
});
...
app.post('/api/arqueo/abrir', async (c) => {
    ...
    if (existe)
        return c.json({ success: false, error: 'Caja ya abierta' }, 400);
